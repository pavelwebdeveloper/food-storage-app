...
function deleteItem(req, res) {
  ...
  pool.query('SELECT amount FROM items WHERE id = $1', [req.query.id], function(err, result) {
	...
	console.log("New req.query.id after if:");
	console.log(req.query.id);
	*/
	
		pool.query('UPDATE items SET amount = $2 WHERE id = $1', [req.query.id, newamount], function(err, result) {
	...
	console.log("items variable:");
	console.log(items);
	successmessage = "Success! You have successfully updated the amount of " + items.itemname;
	const itemname = "";
	const itemamount = 0;
